...
- `remark-dgmo` and the `@diagrammo/dgmo` peer are open `>=X <1` ranges, in step with the other four wrappers; `@docusaurus/core` `^3.0.0`. An open range never re-resolves on its own — bump it on each release and check what the lockfile resolved
- 🔴 **`showcase/package.json` is a second place that has to move, and it is the one that gets forgotten.** It consumes _published_ packages and pins all three **exactly**, so drift shows up in a diff; a stale showcase still builds, so nothing else will tell you
- The `@diagrammo/dgmo` peer floor tracks **remark-dgmo's own peer floor** — never below the dgmo subpaths remark-dgmo imports — or it advertises compatibility our own dependency rules out. npm cannot catch that — nothing validates a peer range against your dependencies' peers. Move the `devDependencies` copy in lockstep, or the tests install a dgmo the floor forbids
